...
class SessionRSVP(models.Model):
    ...
    attended = models.BooleanField(default=False)  # Track actual attendance
    created_at = models.DateTimeField(auto_now_add=True)
...
class SessionMessage(models.Model):
    ...
    def __str__(self):
        return f"{self.sender.username} in {self.session.title}: {self.text[:50]}"


class SessionResource(models.Model):
    """Resources shared by session attendees"""
    session = models.ForeignKey(StudySession, on_delete=models.CASCADE, related_name='resources')
    added_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='shared_resources')
    title = models.CharField(max_length=200)
